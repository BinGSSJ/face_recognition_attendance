...
import numpy as np
import cv2
from config import dir, model_dir ,features_dir # 导入文件夹路径
import time
...
def save_image(img,result,filename,face_name):
    '''
    调用检测人脸数目函数，
    存储人脸图片到相对应的文件夹
    '''
    ...
    if res:
        for i ,d in enumerate(result):
            ...
            right = face.right()
            bottom = face.bottom()
            add_w = int((right - left) / 6)
            add_h = int((bottom - top) / 6)
            ...
            cv2.imwrite(filename +"{0}.jpg".format(face_name),im_blank)
...
if __name__ == '__main__':
    ...
    "http://admin:admin@192.168.1.113:8081/"
    ...
    while cap.isOpened():
        ...
        kk = cv2.waitKey(1)
        faces = detect_face(img_rd)
        # 窗口显示
        # cv2.namedWindow("camera", 0) # 如果需要摄像头窗口大小可调
        if len(faces) != 0:
            if kk == ord('s'):
                print("Sds")
                make_face_dir("wanglihong")
                save_image(img_rd, faces, "wanglihong")
            for k, d in enumerate(faces):
                # 绘制矩形框
                cv2.rectangle(img_rd, tuple([d.rect.left(), d.rect.top()]),
