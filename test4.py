...
from UI.videoaddress import Ui_VideoAddress
...
class Video_Address(QWidget, Ui_VideoAddress):
    def __init__(self):
...
class Attendance(QWidget, Ui_attendance):
    ...
    def __init__(self):
        ...
        self.sin_update.connect(self.update_tablewidget)
    ...
    def update_tablewidget(self,index):
        try:
            ...
            t = students_id_list.index(index)
                ...
                if self.faces_count != 0:
                    ...
                    for k, d in enumerate(faces):
                        ...
                        for i in range(len(id_name)):
                            ...
                            if compare:  # 找到了相似脸
                                if id_name[i][0] not in attendance_id:
                                    attendance_id.add(id_name[i][0])
                                    self.sin_update.emit(id_name[i][0])
